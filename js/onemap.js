...
//suresh
var oneMapGif = "http://uat.onemap.sg/plugin/images/Red_glow.gif";
var host = "http://uat.onemap.sg/";
//var geometryHost = "http://10.0.6.236:6080/";

//var host ="http://119.73.244.163/";
var _baseMapHost = "http://uat.onemap.sg/ArcGIS/rest/services/";
...
function GetOneMap(divName, baseMap, optionalProps) { // by AM
    var i = 1;
    try {
        var center = optionalProps.center;
    } catch (err) {}
    try {
        var level = optionalProps.level;
    } catch (err) {}
    try {
        var layer = optionalProps.layer;
    } catch (err) {}
    ...
    dojo.addOnLoad(function() {
        ...
        var OneMap3 = new esri.Map(_OneMapDiv, {
            extent: startExtent
        });
        ...
        if (baseMap.toUpperCase() == "SM") {
            ...
            var tiledMapServiceLayer = new esri.layers.ArcGISTiledMapServiceLayer(_baseMapURL, {
                tileServers: [_baseMapURL1, _baseMapURL2]
            });
        ...
        NewOneMap.showLocation2 = showLocation2;
        ...
        //newdiv.setAttribute('onclick', function() { window.open('http://uat.onemap.sg/home') });
        newdiv.setAttribute('onclick', "window.open('http://uat.onemap.sg/home')");
...
function showLocation(xVal, yVal) {
    ...
    var xval = xVal;
    var yval = yVal;
    if (isNaN(xval) && isNaN(yval)) {} else {
        var XYsymbol = new esri.symbol.PictureMarkerSymbol(oneMapGif, 20, 20)
        var XYLocation = new esri.geometry.Point(xval, yval, new esri.SpatialReference({
            wkid: 3414
        }))
        var XYGraphic = new esri.Graphic(XYLocation, XYsymbol);
        XYGraphicsLayer.add(XYGraphic);
        var Srchpoint = new esri.geometry.Extent(XYLocation.x - 1000, XYLocation.y - 1000, XYLocation.x + 1000, XYLocation.y + 1000, new esri.SpatialReference({
            wkid: 4326
        }));
...
function showLocation2(xVal, yVal) {
    var OneMap2 = this.map;
    OneMap2.addLayer(XYGraphicsLayer);
    XYGraphicsLayer.clear();
    var xval = xVal;
    ...
    if (isNaN(xval) && isNaN(yval)) {} else {
        ...
        var XYGraphic = new esri.Graphic(XYLocation, XYsymbol);
        XYGraphicsLayer.add(XYGraphic);
        var Srchpoint = new esri.geometry.Extent(XYLocation.x - 10000, XYLocation.y - 10000, XYLocation.x + 10000, XYLocation.y + 10000, new esri.SpatialReference({
            wkid: 4326
        }));
        OneMap2.setExtent(Srchpoint)
    }
...
function overlayKML(kmlFilePath) {
    ...
    if (url.indexOf('160.96.187.80') > 0) {
        url = host + "api/Services.svc/overlayKML?path=" + url
    }
    if (url.indexOf('www.onemap.sg') > 0) {
        url = host + "api/Services.svc/overlayKML?path=" + url
    }
...
function addPointToMap(XY, infoWindowContent, iconURL) {
    ...
    var geomPoint = new esri.geometry.Point(lon, lat, new esri.SpatialReference({
        wkid: 4326
    }));

    var graphic = new esri.Graphic(geomPoint, pointSymbol);
    var npr = new esri.SpatialReference({
        wkid: 3414
    });
    ...
    if (lon.split(".")[0] > 999) {
        ...
        gsvc.project([geomPoint], npr, function(featres) {
            ...
            var XYLocation = new esri.geometry.Point(featres[0].x, featres[0].y, new esri.SpatialReference({
                wkid: 3414
            }));
...
function addPolygonToMap(polyPath, nameValue) {
    ...
    if (polyPath.length > 30) {
        factor = 2
    }
    for (var l = 0; l < polyPath.length; l = l + factor) {

        point1 = new esri.geometry.Point(polyPath[l].split(",")[0].replace('~', ''), polyPath[l].split(",")[1].replace('~', ''), new esri.SpatialReference({
            wkid: 4326
        }))
        pntArr.push(point1)
...
function addPolylineToMap(polyPath, nameValue) {
    ...
    var polygon = new esri.geometry.Polyline(new esri.SpatialReference({
        wkid: 4326
    }));
...
function generateLineGraphic(linePath, color, lineThickness) {
    ...
    for (var l = 0; l < splitLine.length; l++) {
        point1 = new esri.geometry.Point(splitLine[l].split(",")[0], splitLine[l].split(",")[1], new esri.SpatialReference({
            wkid: 3414
        }))
        LineArr.push(point1)
    }
...
function generatePolygonGraphic(polyPath, color, outlineColor, lineThickness) {
    ...
    for (j = 0; j < xyArr.length; j++) {
        poly = new esri.geometry.Point(xyArr[j].split(",")[0], xyArr[j].split(",")[1], new esri.SpatialReference({
            wkid: 3414
        }))
        pntArr.push(poly)
    }
...
function ConvertCoordinate(inputXYList, inputSR, outputSR, callback) {
    ...
    var graphic
    var nsr = new esri.SpatialReference({
        wkid: outputSR
    });

    if (inputXYList.split(",").length == 2) {
        var pointSymbol = new esri.symbol.SimpleMarkerSymbol().setSize(10).setColor(new dojo.Color([255, 0, 0]));
        var geomPoint = new esri.geometry.Point(inputXYList.split(",")[0], inputXYList.split(",")[1], new esri.SpatialReference({
            wkid: inputSR
        }));
        ...
        gsvc.project([graphic], nsr, function(outXY) {
            ...
            if (outputGeoms != "") {
                outputGeoms = outputGeoms.substring(1)
            }
...
function getLineGraphic(linePath, inputSR) {
    ...
    for (var l = 0; l < splitLine.length; l = l + 2) {
        point1 = new esri.geometry.Point(splitLine[l], splitLine[l + 1], new esri.SpatialReference({
            wkid: inputSR
        }))
        LineArr.push(point1)
    }
...
var _OneMapGlobalToken = 'qo/s2TnSUmfLz+32CvLC4RMVkzEFYjxqyti1KhByvEacEdMWBpCuSSQ+IFRT84QjGPBCuz/cBom8PfSm3GjEsGc8PkdEEOEr';
var projSys = '';
